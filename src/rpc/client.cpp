...
namespace rrr
{
    ...
    void UDPClient::end_request()
    {
        ...
        int available = 0;
        uint64_t retry = current_xid%nr_inrings;
        while (
            rte_ring_mp_enqueue(conn->out_bufring[(retry++)%nr_inrings], current_req.get_mbuf()) < 0)
        {
            
            if (retry % 1000000 == 0)
            {
                Log_warn("Stuck in enquueing rpc_request, ring_id =  %d ring size = %d, available = %d tries = %lu", (retry - 1)%nr_inrings,rte_ring_count(conn->out_bufring[(retry-1)%nr_inrings]), 0, retry);
